...
## 스크립트 & 스킬파일 없이 분석할 때의 문제점

이 도구 없이 Claude AI에게 `.eprj` 파일을 직접 분석시키면 다음과 같은 문제가 발생합니다.

### 1. 토큰 폭발 → 세션 만료 (가장 치명적)

| 단계 | 문제 |
|------|------|
| `.eprj`는 SQLite DB | Claude가 직접 바이너리를 열 수 없음 → 사용자가 디코딩 과정을 수동으로 수행해야 함 |
| base64 + gzip 디코딩 후 | 시트당 **수천 줄 JSON-lines** 원본이 대화창에 그대로 들어옴 |
| 멀티시트 프로젝트 | 시트 2개만 해도 원본 데이터가 컨텍스트의 대부분을 차지 |
| 반복 질문 불가 | 1-2번 분석 요청하면 세션 토큰 한도 도달 → **만료** |

### 2. 넷 트레이싱 부정확

스크립트 없이 Claude가 JSON 원본을 눈으로 읽으면:

- **Union-Find 알고리즘 없음** → 다단계 연결(A→B→C→D)을 추적하지 못함
- **T-접합 감지 불가** → 와이어 중간에 닿는 연결을 놓침
- **NET 이름 기반 연결 누락** → 물리적으로 떨어진 같은 이름의 넷을 연결하지 못함
- **좌표 근접성으로 추측** → "가까이 있으니까 연결된 것 같다"는 **오답** 유발

### 3. 핀 좌표 계산 오류

| 항목 | 스크립트 없이 | 스크립트 있을 때 |
|------|-------------|----------------|
| 회전 변환 (0/90/180/270°) | Claude가 수동 계산 → 실수 잦음 | 자동 처리 |
| 미러 처리 | 빠뜨리기 쉬움 | 자동 처리 |
| x-negated 심볼 | 인식 자체를 못함 | fallback으로 자동 보정 |
| `.N` multi-part suffix | 심볼 매칭 실패 | 자동 strip 후 매칭 |

### 4. 파워 심볼 넷 이름 오인

```
심볼 라이브러리 제목: "24v"  ← Claude가 이것을 넷 이름으로 착각
실제 와이어 NET 속성: "+12V" ← 이것이 진짜 넷 이름
```

스크립트 없이는 라이브러리 제목을 넷 이름으로 오인 → **회로 연결이 완전히 잘못 해석됨**

### 5. 저항 MPN 해석 불가

| MPN | Claude 추측 | 실제 값 |
|-----|------------|--------|
| `0603WAF1803T5E` | "?" 또는 매번 데이터시트 검색 | 180kΩ |
| `1206W4F200LT5E` | "200Ω?" | 200mΩ (L=mΩ) |
| `RT0603BRD0750KL` | "750kΩ?" | 50kΩ |

MPN을 하나하나 추측하거나 물어봐야 함 → **분압기 계산이 틀어짐**

### 6. 스킬파일(SKILL.md) 없을 때 추가 문제

| 문제 | 설명 |
|------|------|
| 파일 구조 미인지 | 어떤 테이블에서 데이터를 꺼내야 하는지 모름 |
| 디코딩 방법 반복 설명 | `dataStr` 디코딩 방법을 매 세션마다 다시 안내해야 함 |
| 핵심 규칙 부재 | "좌표 근접성으로 부품-IC 매핑 금지" 같은 규칙이 없어 오분석 |
| 미배치 심볼 혼입 | IC pin maps에서 라이브러리에만 있는 심볼을 걸러내지 못함 |
| 비일관적 출력 | 분석 결과 형식(IC 블록별 그룹화 등)이 세션마다 달라짐 |

### 요약

```
스크립트 없음 → 토큰 폭발 + 부정확한 분석 + 세션 만료
스킬파일 없음 → 매 세션마다 처음부터 학습 + 핵심 규칙 누락
둘 다 없음   → 사실상 .eprj 분석 불가능
둘 다 있음   → 정확한 분석 + 토큰 절약 + 세션 간 일관성 유지
```
