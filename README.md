...
| 사용 방법 | Python 실행 위치 | 설명 |
|-----------|-----------------|------|
| **Claude AI 프로젝트 스킬** | Claude 서버 (샌드박스) | `/mnt/skills/user/` 경로에 등록, 파일 업로드 시 서버에서 자동 실행 |
| **Claude Code (CLI/IDE)** | 내 컴퓨터 (로컬) | 로컬 터미널에서 직접 실행 |
| **독립 실행** | 내 컴퓨터 (로컬) | Claude 없이 커맨드라인에서 단독 사용 |

Claude AI 프로젝트 스킬로 등록하면 사용자가 `.eprj` 파일을 업로드할 때 **Claude 서버의 샌드박스 환경**에서 Python이 실행되고, 구조화된 결과만 LLM에 전달됩니다. 사용자의 PC에서 실행되는 것이 아닙니다.

| 항목 | 원본 직접 전달 | 스크립트 사용 |
|------|---------------|-------------|
| 토큰 소비 | 시트당 수만 토큰 | ~50줄 요약 |
| 연산 부담 | Claude가 JSON 파싱 + 좌표 계산 | Python 스크립트가 처리 |
| 정확도 | 좌표 기반 추정 (오류 가능) | Union-Find 기반 정확한 넷 추적 |
| 세션 지속 | 1-2회 분석으로 만료 | 반복 분석 가능 |

MCP 서버가 외부 도구를 호출하고 결과만 LLM에 전달하는 것처럼, 이 스크립트도 **무거운 데이터 처리는 Python 스크립트가 담당하고, Claude는 구조화된 결과의 해석과 설계 판단에 집중**하는 구조입니다.
